<template>
	<view class="login">
		<view class="content">
			...
			<!-- 主体表单 -->
			<view class="main">
				<wInput v-model="account" type="text" placeholder="请输入账号" :focus="isFocus"></wInput>
				<wInput v-model="passData" type="password" placeholder="请输入密码"></wInput>
				<wInput v-model="rePassData" type="password" placeholder="请再次确认密码"></wInput>
				<view class="veri-info">
					<wInput v-model="veriData" type="text" placeholder="请输入验证码" showPassword=false></wInput>
					<u-image @click="verifCodeEvent" class="veri-image" width="200rpx" height="92rpx" :src="captchaSrc"></u-image>
					<!-- <u-image @click="verifCodeEvent" class="veri-image" style="width: 200rpx;height: 92rpx;" :src="url" mode=""></u-image> -->
				</view>

			</view>
			<wButton class="wbutton" text="修 改" :rotate="isRotate" @click="startLogin"></wButton>
...
<script>
	...
	export default {
		...
		methods: {
			// 获取验证码
			async verifCodeEvent() {
				const time = new Date() * 1
				await this.$u.api.getveri({
					time
				}).then(res => {
					const data = res.data
					this.url = 'data:image/jpeg;base64,' + data.img
					this.kaptchaId = data.kaptchaId
				})
				// await uni.request({
				// 	url: baseurl + '?time=' + time, //仅为示例，并非真实接口地址。
				// 	method: "GET",
				// 	success: (res) => {
				// 		// console.log(res);
			...
			async startLogin(e) {
				...
				this.passData = new MD5().update(this.passData + '').digest('hex').toUpperCase()
				if (this.veriData.length == "") {
					uni.showToast({
						icon: 'none',
						position: 'bottom',
						title: '请输入验证码'
				...
				_this.isRotate = true
				const loginForm = {
					sourceId: this.account,
					credentials: this.passData,
					loginUserType: "fm",
					loginVerifCode: this.veriData,
					kaptchaId: this.kaptchaId
				}
				await this.$u.api.login(loginForm).then(res => {
					console.log(res);
				...
				await this.$u.api.login(loginForm2).then(res => {
					res.data = JSON.parse(res.data)
					console.log(res);
					setUserName(res.data.userName)
					setUserCity(res.data.officeAddress)
